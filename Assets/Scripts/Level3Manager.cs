...
public class Level3Manager : MonoBehaviour
{
    ...
    public void OnColorPressed(int index)
    {
        ...
        if (sequence[playerIndex] == index)
        {
            ...
            if (playerIndex >= sequence.Count)
            {
                score += 4;
                UpdateUI();
                if (sequence.Count < 6) { AddNewColor(); StartCoroutine(ShowSequence()); }
                else { LevelCompleted(); }
            }
        }
        else { GameOver(); }
    ...
    void LevelCompleted() { resultPanel.SetActive(true); resultText.text = "Tebrikler!"; }
    void GameOver() { resultPanel.SetActive(true); resultText.text = "Oyun Bitti!"; }
