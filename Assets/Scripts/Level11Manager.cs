...
public class Level11Manager : MonoBehaviour
{
    ...
    [Header("Buttons")]
    public Button[] colorButtons; // Yer değiştirecek olanlar
    ...
    private int level = 11; // Bu sahnenin seviyesi
    private int score = 0;
    ...
    void Start()
    {
        ...
        retryButton.onClick.AddListener(() => SceneManager.LoadScene("Level11"));
    ...
    void CheckNextLevelLock()
    {
        ...
        int nextLevelNumber = level + 1; // Seviye 11 için sonraki seviye 12'dir

        // Eğer sonraki seviye (Level 12) halihazırda önceden açılmışsa buton aktiftir
    ...
    {
        SceneManager.LoadScene("Level" + (level + 1)); // Doğrudan Level12 sahnesini yükler
    }
    ...
    IEnumerator ShowSequence()
    {
        ...
        // --- SEVİYE 11 YENİLİĞİ: RENK BUTONLARINI KARIŞTIR ---
        ShuffleColorButtons();
    ...
    // Renk butonlarının hiyerarşideki yerini değiştirerek ekranda yer değiştirmelerini sağlar
    void ShuffleColorButtons()
    {
        ...
        for (int i = 0; i < colorButtons.Length; i++)
        {
            ...
            colorButtons[i].transform.SetSiblingIndex(randomIndex);
        }
    }
    ...
    {
        int reachedLevel = PlayerPrefs.GetInt("ReachedLevel", 1);
        int currentLevelNum = 11;

        if (reachedLevel <= currentLevelNum)
        {
            PlayerPrefs.SetInt("ReachedLevel", currentLevelNum + 1); // Seviye 12 kilidini açar
            PlayerPrefs.Save();
            Debug.Log("Sistem: Seviye 12 kilidi başarıyla açıldı ve kaydedildi.");
        }
        ...
        resultText.text = "Efsane! Seviye " + currentLevelNum + " Tamamlandı.";
    }

    void GameOver()
    {
        // Seviye 12 önceden açıldıysa aktif kalır, açılmadıysa pasif kalır.
        resultPanel.SetActive(true);
